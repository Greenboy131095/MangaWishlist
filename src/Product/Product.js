...
const Product = ({id}) => {
  const history = useNavigate();
   ...
   useEffect(()=>{
    ...
    function findManga(mangaName){
      return mangaName.id===id;
    }

    let variable=manga.find(findManga);
  ...
  return (
    ...
    <details>
     <summary><b>Manga Details</b></summary>
        <ul>
          
          <li><b>Summary: </b>{variable.description}</li>
          <li><b>Alternate Name: </b>{variable.alternatename}</li>
          <li><b>Rating:4.5</b></li>

          
        </ul>
    </details>
    <form onSubmit={(e)=>submit(e)}>
    <p><button className="button1"to="/category/Wishlists">Add To Wishlist</button></p>
    </form>
